<template>
  <div
    ...
    :style="{ 'grid-template-columns': isMobile ? '1fr' : '1fr' }"
  >
    <!-- Grid Tengah: Canvas 4R -->
    <div
      v-if="selectedFrame"
      class="py-4 rounded-lg flex flex-col items-center relative"
    >
      <h2 class="text-2xl font-semibold text-gray-700 mb-10 mt-[-0.5rem]">
        Preview
      </h2>

      <div
        id="canvas"
        class="bg-gray-100 rounded-lg border-gray-300 w-[360px] h-[540px] flex items-center justify-center relative"
        ref="canvas"
      >
    ...
    <div
      v-if="!selectedFrame"
      class="p-2 rounded-lg flex flex-col overflow-y-auto"
    >
      <h2 class="text-lg md:text-2xl font-semibold mb-6 text-gray-700 ml-12">
        Pilih Frame
      </h2>
  ...
  <Button
    v-if="selectedFrame"
    class="fixed z-10 bottom-6 right-6 py-3 px-6 bg-blue-500 text-white rounded-full shadow-lg text-lg"
    @click="handlePrint"
...
const ipEmail = import.meta.env.VITE_SERVER_IP_EMAIL;
...
// Function to update the isMobile flag when window is resized
...
const handlePrint = async () => {
  ...
  const targetDPI = 350; // Desired DPI
  const targetWidthInInches = 4; // Width in inches (example)
  const targetHeightInInches = 6; // Height in inches (example)

  // Calculate the target resolution based on DPI
  const targetWidth = targetDPI * targetWidthInInches; // Width in pixels
  const targetHeight = targetDPI * targetHeightInInches; // Height in pixels

  // Scale calculation for print
  const scaleX = targetWidth / originalWidth;
  const scaleY = targetHeight / originalHeight;
  const scale = Math.min(scaleX, scaleY); // Maintain aspect ratio
  console.log("Scale for 350 DPI:", scale);
  ...
  try {
    ...
    const canvasDataUrl = canvas.toDataURL("image/jpeg", 1); // Convert the canvas to a JPEG data URL
    ...
    formData.append("file", blob, `${orderId.value}-${frameName.value}.jpg`);
    formData.append("email", emailStored); // Add the email as a parameter

    // Send the image to the server for processing (via /send-edit)
    const response = await fetch(`http://${ipEmail}/send-edit`, {
      method: "POST",
      body: formData,
    });
    ...
    if (response.ok) {
      ...
      // Send to /print (for actual printing) - this part remains unchanged
      const formDataForPrint = new FormData();
      formDataForPrint.append(
        "file",
        blob,
        `${orderId.value}-${frameName.value}.jpg`
      );

      const printResponse = await fetch(`http://${ip}/print`, {
        method: "POST",
        body: formDataForPrint,
      });

      if (printResponse.ok) {
        console.log("File sent to print successfully.");
      } else {
        console.error("Error sending file to print:", printResponse);
      }
    } else {
